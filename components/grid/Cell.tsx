...
export const Cell = ({ value, status }: Props) => {
  ...
  useEffect(() => {
    if (!status) setClasses('bg-slate-900 border-slate-600 text-white')
    if (status === 'absent') setClasses('bg-slate-700 text-white border-slate-700')
    if (status === 'correct') setClasses('bg-green-600 text-white border-green-600')
